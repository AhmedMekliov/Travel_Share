...
@Repository
public class SearchPostDaoJDBC implements SearchPostDao {

	String findPostsQuery = "select * from posts where posts.start_dest = ? and posts.end_dest = ? and posts.start_date = ?";
	String deleteQuery = "delete from posts where posts.post_id = ?";
	String findPostByEmail = "select * from posts where posts.user_email = ?";
	...
	public List<Post> findPostsByEmail(String email) {
		...
		jdbcTemplate.query(findPostByEmail, new Object[] { email },
				new ResultSetExtractor<Post>() {
